...
def enrich_df_trials_fm(df_trials_fm):
    ...
    for i_iter, (ses_idx, model_name) in enumerate(itertools.product(sessions[:], models)):
        ...
        for i_mod, mod in enumerate(models):
            ...
            # to fix: should reward be earned reward?
            #         extra reward (i.e. manual reward and auto-rewards)
            df_ses.loc[:, ['RPE_earned']] = df_ses['earned_reward'].astype(float) - chosen_values  # noqa: E501
            df_ses.loc[:, ['RPE_all']] = (df_ses['earned_reward'].astype(float) + df_ses['extra_reward'].astype(float)) - chosen_values  # noqa: E501
