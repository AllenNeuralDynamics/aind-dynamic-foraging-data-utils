...
def extract_time_window(trial_i, timepoints, alignment_events,
                        offsets, absolute_time, align_in_trial):
    """
...
def extract_signal_window(trial_i, timepoints, varpoints,
    ...
    for (ses_idx, event), df_fip_i in df_fip_z.groupby(['ses_idx', 'event']):
        ...
        for var_event in ['data', 'data_z']:
            ...
            df_trials_fip.loc[df_trials_fip['ses_idx'] == ses_idx, f'{var_event}_{event}'] = (
                df_trials_ses.apply(
                    lambda trial_i: extract_signal_window(trial_i, timepoints, varpoints,
                                                          alignment_events, offsets, absolute_time),
                    axis=1
                )
            )
...
def tidy_df_trials_fip(df_fip, df_trials_fip, col_prefix_signal='data'):
    ...
    timestamp_cols = [col for col in df_trials_fip.columns
                      if col.startswith("timestamps_in_session")]
    data_cols = [col for col in df_trials_fip.columns
    ...
    for timestamp_col in timestamp_cols:
        ...
        rename_dict = {data_col: data_col.replace(f"_{event}_norm", "_norm")
