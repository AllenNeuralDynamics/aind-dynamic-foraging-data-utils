...
def create_df_session(nwb_filename):
    ...
    df_session['ses_idx'] = df_session['subject_id'].values + '_' + df_session['session_date'].values
...
def create_df_trials(nwb_filename):
    key_from_acq = ['left_lick_time', 'right_lick_time', 'left_reward_delivery_time', 'right_reward_delivery_time', 'FIP_falling_time', 'FIP_rising_time']
    io = NWBHDF5IO(nwb_filename, mode='r')
    nwb = io.read()
    ...
    for col in df_ses_trials.columns:
        if ('time' in col) and (col != 'goCue_start_time'):
            df_ses_trials.loc[:,col] = df_ses_trials[col].values-df_ses_trials['goCue_start_time'].values
    df_ses_trials['goCue_time_absolute'] = absolute_time
    df_ses_trials['goCue_start_time'] = 0.        
    events_ses = {key: nwb.acquisition[key].timestamps[:] - t0 for key in key_from_acq}        
    
    for event in ['left_lick_time', 'right_lick_time', 'left_reward_delivery_time','right_reward_delivery_time']:
        event_times = events_ses[event]    
        df_ses_trials[event] = df_ses_trials.apply(lambda x: np.round(event_times[(event_times>(x['goCue_start_time']+x['goCue_time_absolute'])) & (event_times<(x['stop_time']+x['goCue_time_absolute']))]-x['goCue_time_absolute'],4), axis=1)        

    df_ses_trials['reward_time'] = df_ses_trials.apply(lambda x: np.nanmin(np.concatenate([[np.nan],x['right_reward_delivery_time'],x['left_reward_delivery_time']])), axis=1)
    df_ses_trials['choice_time'] = df_ses_trials.apply(lambda x: np.nanmin(np.concatenate([[np.nan],x['right_lick_time'],x['left_lick_time']])), axis=1)
    df_ses_trials['reward'] = (df_ses_trials.rewarded_historyR.astype(int) | df_ses_trials.rewarded_historyL.astype(int))
    df_ses_trials = df_ses_trials.drop(columns=['left_lick_time', 'right_lick_time', 'left_reward_delivery_time','right_reward_delivery_time'])    
    ...
    if interpolate:
        ...
        for event_number, event_time in enumerate(np.array(event_times)):
            ...
            # on the linearly spaced time array
                ...
                data_dict.update({
                    'event_{}_t={}'.format(
                        event_number, event_time): np.interp(
                        data_dict['time'],
