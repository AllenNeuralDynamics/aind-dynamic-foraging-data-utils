...
behavior_746345_2024-11-18_09-46-17_processed_2024-11-19_19-15-06:
             9b6d248d-ad7b-49dd-b8d7-d4549e87e722
behavior_746345_2024-12-04_09-16-20_processed_2025-01-04_00-49-53:
            ff364262-0e04-4cc9-a34a-41083fec1213
...
def get_all_df_for_nwb(filename_sessions, loc="../scratch/", interested_channels=None):
    get_all_df_for_nwb gets all the dataframes for the NWB and saves it
    iteratively onto a location in '../scratch/' or a specified location
    example: get_all_df_for_nwb(filename_sessions, loc =
                '../scratch/kenta_dec2/', interested_channels = interested_channels)
    if not os.path.exists(loc):
        os.makedirs(loc)
        print(f"Directory created: {loc}")
    else:
...
def enrich_df_trials_fm(df_trials_fm):
    """
    enrich_df_trials_fm: enriches the df_trials_fm with additional columns
    like RPE_earned, RPE_all, Q_chosen, Q_unchosen, Q_sum, Q_Delta, Q_change
    ...
    for i_iter, (ses_idx, model_name) in enumerate(itertools.product(sessions[:], models)):
        ...
        for i_mod, mod in enumerate(models):
            ...
            # to fix: should reward be earned reward? extra reward
            # # (i.e. manual reward and auto-rewards)
